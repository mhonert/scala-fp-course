...
class BloxorzSuite extends FunSuite {
  ...
  test("find neighbors for current block") {
    new Level1 {
      val block = Block(Pos(1, 1), Pos(1, 1))
      val neighbors = neighborsWithHistory(block, List(Left, Up))
  ...
  test("find only new neighbors for current block") {
    new Level1 {
  ...
  test("find solution in infinite terrain") {
    new InfiniteLevel {
