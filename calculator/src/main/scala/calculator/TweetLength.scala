...
object TweetLength {
  ...
  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = {
    Signal(colorForRemainingCount(remainingCharsCount()))
  }

  final val ColorSafe = "green"
  final val ColorCloseToLimit = "orange"
  final val ColorLimitExceeded = "red"

  def colorForRemainingCount(count: Int): String =
    if (count >= 15) ColorSafe
    else if (count < 0) ColorLimitExceeded
    else ColorCloseToLimit
